...
function SemVer(version, loose) {
    ...
    var arrayUsingItr2 = m[4].split('.');
    var newArray2 = [];
    for (var i2 = 0, counter2 = 0; i2 < arrayUsingItr2.length; i2++) {
        var id = arrayUsingItr2[i2];
        if (/^[0-9]+$/.test(id)) {
            var num = +id;
            if (num >= 0 && num < MAX_SAFE_INTEGER)
                return num;
        }
        newArray2[counter2] = num;
        counter2++;
    }
    if (!m[4])
        this.prerelease = [];
    else
        this.prerelease = m[4].split(newArray2);
    ;
...
function Range(range, loose) {
    ...
    this.set = newArray2.filter(function (c) {
        return c.length;
    });
    if (!this.set.length) {
...
function toComparators(range, loose) {
    ...
    for (var i2 = 0, counter2 = 0; i2 < arrayUsingItr2.length; i2++) {
        ...
        newArray2[counter2] = newArray.join(' ').trim().split(' ');
        counter2++;
    }
    return newArray2;
}
