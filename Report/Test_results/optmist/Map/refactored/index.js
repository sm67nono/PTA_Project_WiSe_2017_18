...
function Argv(processArgs, cwd) {
    ...
    if (!cwd)
        cwd = process.cwd();
    var arrayUsingItr2 = process.argv.slice(0, 2);
    var newArray2 = [];
    var arg2 = null;
    var counter2 = 0;
    for (var i2 = 0; i2 < arrayUsingItr2.length; i2++) {
        var x = arrayUsingItr2[i2];
        var b = rebase(cwd, x);
        newArray2[counter2] = x.match(/^\//) && b.length < x.length ? b : x;
        counter2++;
    }
    self.$0 = newArray2.join(' ');
    ...
    self.help = function () {
        ...
        var arrayUsingItr = Object.keys(switches);
        var newArray = [];
        var arg = null;
        var counter = 0;
        for (var i = 0; i < arrayUsingItr.length; i++) {
            var s = arrayUsingItr[i];
            newArray[counter] = switches[s] || '';
            counter++;
        }
        var switchlen = longest(newArray);
        var arrayUsingItr = Object.keys(descriptions);
        var newArray = [];
        var arg = null;
        var counter = 0;
        for (var i = 0; i < arrayUsingItr.length; i++) {
            var d = arrayUsingItr[i];
            newArray[counter] = descriptions[d] || '';
            counter++;
        }
        var desclen = longest(newArray);
        keys.forEach(function (key) {
...
function rebase(base, dir) {
    ...
    ds.splice(0, i);
    bs.splice(0, i);
    var arrayUsingItr2 = bs;
    var newArray2 = [];
    var arg2 = null;
    var counter2 = 0;
    for (var i2 = 0; i2 < arrayUsingItr2.length; i2++) {
        var val2 = arrayUsingItr2[i2];
        newArray2[counter2] = '..';
        counter2++;
    }
    var p = path.normalize(newArray2.concat(ds).join('/')).replace(/\/$/, '').replace(/^$/, '.');
    return p.match(/^[.\/]/) ? p : './' + p;
}
;